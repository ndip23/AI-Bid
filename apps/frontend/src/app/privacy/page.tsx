...
import { ShieldCheck, Lock, Eye, FileText, CheckCircle2, LayoutDashboard, ArrowRight } from 'lucide-react';
...
export default function PrivacyPolicyPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 md:px-10 pt-24 pb-16 space-y-6 w-full">
        {/* Authenticated User Return to Dashboard Banner */}
        {user && (
          <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-emerald-950 shadow-sm animate-fade-in">
            <div className="flex items-center space-x-2 text-xs sm:text-sm font-bold">
              <ShieldCheck className="w-4 h-4 text-emerald-700 shrink-0" />
              <span>
                {isFrench ? 'Session Active :' : 'Active Session:'}{' '}
                <strong className="text-emerald-900 font-extrabold">{company?.name || user?.email}</strong>
              </span>
            </div>
            <Link
              href="/dashboard"
            ...
            >
              ...
              <span>{isFrench ? 'Retourner au Tableau de Bord' : 'Return to Dashboard'}</span>
